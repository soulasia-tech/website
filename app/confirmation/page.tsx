        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <Card className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-lg text-center">
                    <h2 className="text-2xl font-bold mb-4 text-red-600">There was a problem with your payment or
                        reservation</h2>
                    <div className="mb-2 text-base text-gray-700">Reservation was not complete. Please try again
                        later.
                    </div>
                    {errorMessage && <div className="mb-2 text-sm text-red-500">{errorMessage}</div>}
                    <div className="mt-4 text-sm text-gray-500">Need help? Contact us at <a
                        href="mailto:info@soulasia.com.my" className="text-blue-600 underline">info@soulasia.com.my</a>
                    </div>
                </Card>
            </div>
        );
    ...
    if (loading || paymentStatus !== 'succeeded' || reservationStatus !== 'succeeded') {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center">
                <svg className="animate-spin h-8 w-8 text-blue-600 mb-4" xmlns="http://www.w3.org/2000/svg" fill="none"
                     viewBox="0 0 24 24">
        ...
        <div className="min-h-screen py-12">
            <div className="container mx-auto px-4 flex flex-col items-center">
                <Card className="bg-white rounded-2xl shadow-lg p-6 w-full mb-8">
                    ...
                    {/* Booking Reference */}
                    {bookingToken && (
                        <div className="mb-4 text-sm text-gray-500 text-left">Booking Reference: <span
                            className="font-mono text-gray-700">{bookingToken}</span></div>
                    )}

                    {/* Details Grid - Multi-apartment support */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 text-left mb-4">
                        <div><span className="font-medium text-gray-600">Guest:</span> <span
                            className="font-semibold">{guestName}</span></div>
                        <div><span className="font-medium text-gray-600">Email:</span> <span
                            className="font-semibold">{guestEmail}</span></div>
                        <div><span className="font-medium text-gray-600">Property:</span> <span
                            className="font-semibold">{propertyName}</span></div>
                        <div>
                            <div><span className="font-medium text-gray-600">Check-in:</span> <span
                                className="font-semibold">{checkIn}</span></div>
                            <div><span className="font-medium text-gray-600">Check-out:</span> <span
                                className="font-semibold">{checkOut}</span></div>
                        </div>
                    ...
                    <div className="mt-4 text-sm text-gray-500 text-left">
                        Need help? Contact us at <a href="mailto:info@soulasia.com.my"
                                                    className="text-blue-600 underline">info@soulasia.com.my</a>
                    </div>
                </Card>
