...
function SearchResults() {
    ...
    useEffect(() => {
        ...
        const fetchPropertiesAndRooms = async () => {
            ...
            setError(null);
            try {
                console.log('Fetching properties and rooms...', {
                    city,
                    propertyIdParam,
                    startDate,
                    endDate,
                ...
                if (propertyIdParam) {
                    filteredProperties = propData.properties.filter((p: {
                        propertyId: string
                    }) => p.propertyId === propertyIdParam);
                } else {
                ...
                setProperty(propertyIdParam ? {
                    propertyId: propertyIdParam,
                    propertyName: filteredProperties[0].propertyName
                } : {propertyId: '', propertyName: city});
                ...
                await Promise.all(filteredProperties.map(async (property: {
                    propertyId: string;
                    propertyName: string;
                    city: string
                }) => {
                    ...
                    if (roomData.success && Array.isArray(roomData.roomTypes)) {
                        ...
                        if (roomsData.success && Array.isArray(roomsData.rooms)) {
                            roomsData.rooms.forEach((room: unknown) => {
                                if (!propertyRoomsByType[(room as {
                                    roomTypeID: string
                                }).roomTypeID]) propertyRoomsByType[(room as { roomTypeID: string }).roomTypeID] = [];
                                propertyRoomsByType[(room as { roomTypeID: string }).roomTypeID].push((room as {
                                    roomID: string
                                }).roomID);
                            });
                        }
        ...
        uniquePropertyIds.forEach(propertyId => {
            if (!propertyInfoData[propertyId] && !propertyInfoLoading[propertyId]) {
                ...
                fetch(`/api/cloudbeds/property?propertyId=${propertyId}`)
                    ...
                    .catch(() => {
                        setPropertyInfoData(prev => ({
                            ...prev,
                            [propertyId]: {error: 'Failed to load property information'}
                        }));
                    })
                    .finally(() => {
                        ...
                        <div className="space-y-4 tb:space-y-5">
                            {!error && filteredRooms.length === 0 && (
                                <div className="text-gray-500 text-center py-8">
                                    No rooms found for your search criteria. Try another property or reduce the number
                                    of guests.
                                </div>
                            )}
                                    ...
                                    <div key={room.id}
                                        ...
                                        <div className="flex flex-col space-y-2.5 tb:space-y-5 w-full lp:min-w-[450px]">
                                            <div className="gap-1 tb:gap-2.5">
                                                ...
                                                <div
                                                    className="text-base tb:text-lg text-[#4a4f5b] font-normal">{room.propertyName}</div>
                                                <div className="text-base tb:text-lg text-[#4a4f5b] font-normal">Up
                                                    to {room.maxGuests} guests
                                                </div>
                                            </div>
                                            ...
                                            <div
                                                ...
                                                <div className="w-full flex flex-row gap-2.5 justify-between flex-1">
                                                    ...
                                                    <div className="flex flex-col gap-2.5">
                                                        <span
                                                            className="text-xs lp:text-sm text-gray-500 mb-0.5">Apartments</span>
                                                    ...
                                                    <div className="flex flex-col gap-2.5">
                                                        <span
                                                            className=" text-xs lp:text-sm text-gray-500 mb-0.5">Adults</span>
                                                        <div className="flex items-center gap-2">
                                                            <Button type="button" size="responsive" variant="outline"
                                                                    className="text-lg tb:text-2xl size-[var(--action-h-sm)] tb:size-[var(--action-h-md)]"
                                                    ...
                                                    <div className="flex flex-col gap-2.5">
                                                        <span
                                                            className="text-xs lp:text-sm text-gray-500 mb-0.5">Children</span>
                                                        <div className="flex items-center gap-2">
                                                            <Button type="button" size="responsive" variant="outline"
                                                                    className="text-lg tb:text-2xl size-[var(--action-h-sm)] tb:size-[var(--action-h-md)]"
                                                                    onClick={() => setRoomGuests(g => ({
